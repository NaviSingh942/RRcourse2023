...
```{r}
episodes_nop <- nrow(season_data)

avg_viewers <- mean(season_data$viewers)

nr_of_viewers_beg <- season_data$viewers[1]

nr_of_viewers_end <- season_data$viewers[episodes_nop]

premiere_date_beg <- season_data$premiere_date[1]

premiere_date_end <- season_data$premiere_date[episodes_nop]

most_popular_episode <- max(season_data$viewers)

most_popular_episode_title <- season_data$title[season_data$viewers==most_popular_episode]

most
```

Season `r params&season` of Game of Thrones consisted of `r episodes_nop` episodes that aired between `premiere_date_beg` and `premiere_date_end` on HBO. The show gathered an average of `r `avg_viewers` first-day TV viewers in the US, with the number growing from `r nr_of_viewers_beg` to `r nr_of_viewers_beg` million by the end of the season.

The most popular episode of the season was `most_popular_episode`, in which:
